...
int main(int argc, char* argv[]) {
    ...
    if (val != 42) {
        printf("FAIL: Byte test - wrote 42, read %" PRIu32 "\n", val);
        return 1;
    }
    ...
    if (val != 0xDEADBEEF) {
        printf("FAIL: Int test - wrote 0xDEADBEEF, read 0x%" PRIx32 "\n", val);
        return 1;
    }
    ...
    if (val != 0x11) {
        printf("FAIL: Byte at offset 10 - wrote 0x11, read 0x%" PRIx32 "\n", val);
        return 1;
    }

    val = w2c_memory_read_byte(&instance, 11);
    if (val != 0x22) {
        printf("FAIL: Byte at offset 11 - wrote 0x22, read 0x%" PRIx32 "\n", val);
        return 1;
    }

    val = w2c_memory_read_byte(&instance, 12);
    if (val != 0x33) {
        printf("FAIL: Byte at offset 12 - wrote 0x33, read 0x%" PRIx32 "\n", val);
        return 1;
    }
    ...
    if (val != 0x01) {
        printf("FAIL: First byte of int - expected 0x01, got 0x%" PRIx32 "\n", val);
        return 1;
    }

    val = w2c_memory_read_byte(&instance, 21);
    if (val != 0x02) {
        printf("FAIL: Second byte of int - expected 0x02, got 0x%" PRIx32 "\n", val);
        return 1;
    }
